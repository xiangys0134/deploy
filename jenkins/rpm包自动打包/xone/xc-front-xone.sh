...
function build_pkg() {
    ver='1.0.0.1'
    sudo rm -rf ${JENKINS_HOME}/rpmbuild/RPMS/x86_64/${spec_file%%.spec}*.rpm
...
function upload_file() {
        ...
        sudo chown -R ftp. ${ftp_dir}/XONE/${JOB_NAME}
...
function git_upload() {
    ...
    git_url=git@192.168.0.38:ops/xc-front-xone-pack.git
    gir_dir=`echo ${git_url}|awk -F [/.] '{print $(NF-1)}'`
    if [[ "${release_tag}" =~ "origin" ]] || [[ "${release_tag}" =~ "xc-front-xone" ]]; then
