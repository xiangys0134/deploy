...
function redis_install() {
    ...
    #yum install yum-utils -y && yum-config-manager --add-repo  http://mirrors.xuncetech.com/xunce/xunce-dev/yum/el7/xunce-dev.repo
    if [ `check_rpm redis` == '0' ]; then
        epel_install
        rpm -ivh https://www.rpmfind.net/linux/epel/7/x86_64/Packages/j/jemalloc-3.6.0-1.el7.x86_64.rpm
        #yum --enablerepo=xunce-dev install -y redis-4.0.10 >/dev/null >&1
    ...
    [ ! -d ${redis_data}/conf ] && mkdir -p ${redis_data}/conf
    [ ! -d ${redis_data}/data ] && mkdir -p ${redis_data}/data
    [ ! -d ${redis_data}/log ] && mkdir -p ${redis_data}/log
