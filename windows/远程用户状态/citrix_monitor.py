...
citrix_http = 'http://192.168.0.63'
citrix_log = 'citrix_monitor.log'
citrix_file_dir = r'C:\bat_logs'
...
if __name__ == '__main__':
    while True:
        try:
            ...
            if not os.path.exists(citrix_file_dir):
                #print('aaaa')
                os.system(os.makedirs(citrix_file_dir))
            # url = 'admin'
            if url == 0 or url == 'None':
                print('无远程用户登录')
                with open(os.path.join(citrix_file_dir,'citrix_monitor.log'),'a+',encoding='utf8') as f:
                    f.seek(2)
                    f.write('\n%s 无远程用户登录' %time.strftime('%Y-%m-%d %X',time.localtime(time_date)))
                time.sleep(3)
                continue
            http_url = '%s/pub-uds/monitor/virtual_desk?username=%s' %(citrix_http,url)

            print(url,http_url)
            ret = request.Request(url=http_url,headers=header_dict)
            res = request.urlopen(ret,timeout=2)
            result = res.read().decode('utf8')
            with open(os.path.join(citrix_file_dir, citrix_log), 'a+', encoding='utf8') as f1:
                f1.seek(2)
                f1.write('\n%s %s  %s' % (time.strftime('%Y-%m-%d %X',time.localtime(time_date)),str(result),http_url))
            print(result)
        except Exception as e:
            with open(os.path.join(citrix_file_dir, citrix_log), 'a+', encoding='utf8') as f2:
                f2.seek(2)
